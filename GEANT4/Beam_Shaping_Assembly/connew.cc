...
// --- بناء مرشح النيوترونات السريعة (Nickel Filter Box) ---
   ...
   logicFastFilter =
    new G4LogicalVolume(solidFastFilter, nickel, "logicFastFilter");
...
// 1. أولاً: بناء العاكس (هنا تضعين الكود الجديد)
  
// ==========================================
    // بناء العاكس المحيط وقناة دخول البروتون (Kim-like Reflector with Beam Entrance Channel)
    // ==========================================
    ...
    // بداية ونهاية الـ BSA الحقيقية على محور Z
    ...
    // امتداد العاكس ليغطي من أسفل منطقة دخول البروتون وحتى أعلى الـ BSA
    G4double reflectorExtra = 1.0 * cm;
    
    // جعل العاكس يمتد للأسفل أكثر ليكون محيطاً بقناة دخول البروتون
    G4double beamEntranceLength = 3.0 * cm; // طول قناة دخول البروتون المحاطة بالرصاص
    G4double reflectorBottomZ = xyzTarget.z() - targetZ - beamEntranceLength;
    ...
    // 3. قناة دخول البروتون والهدف (Beam Entrance Channel & Target Space)
    // ترك مساحة مريحة (Clearance) حول مسار الشعاع والهدف لضمان عدم اعتراض البروتونات بواسطة الرصاص
    G4double beamChannelClearance = 0.5 * cm;
    G4double beamChannelHalfZ = reflectorHalfLength; // امتداد القناة عبر كامل العاكس من الأسفل للهدف
    
    G4Box *solidBeamChannel = new G4Box("solidBeamChannel", 
                                        targetX + beamChannelClearance, 
                                        targetY + beamChannelClearance, 
                                        beamChannelHalfZ);
                                        
    // موضع قناة الفراغ لتكون متمركزة مع مسار شعاع البروتون والهدف وتفتح نحو الأسفل
    G4double beamChannelCenterZ = xyzTarget.z() - reflectorCenterZ;
    G4ThreeVector beamChannelSubPos(0., 0., beamChannelCenterZ);

    // عمليات الطرح الهندسية للـ CSG لتشكيل العاكس النهائي مع القناة
    G4SubtractionSolid *interReflector = new G4SubtractionSolid("interRef", solidOuterReflector, solidInnerBsaSpace, 0, bsaSubPos);
    G4SubtractionSolid *solidReflector = new G4SubtractionSolid("solidReflector", interReflector, solidBeamChannel, 0, beamChannelSubPos);
   ...
   // ==========================================
/*    G4double fullBsaLength = hzModerator*2.0 + hzFastFilter*2.0 + hzGammaFilter*2.0 + hzCollimator*2.0;
    G4double halfBsaLength = fullBsaLength / 2.0; 

    G4ThreeVector xyzReflector(0., 0., xyzModerator.z() - hzModerator + halfBsaLength);

    G4VSolid *solidOuterReflector = new G4Box("solidOuterReflector", bsaX + reflectorThickness, bsaY + reflectorThickness, halfBsaLength);
    G4VSolid *solidInnerReflectorSpace = new G4Box("solidInnerReflectorSpace", bsaX, bsaY, halfBsaLength + 1.*mm);    
    ...
    */
    ...
    // بناء كاشف الـ BF3 ليطابق تماماً فتحة الخروج للموازئ (Radius = 7 cm)
    G4double detectorRadius = 7.0 * cm; 
    G4double detectorHalfLength = 1.0 * cm; // سماكة صغيرة كافية للـ scoring عند المخرج
    ...
    // وضعه مباشرة عند مخرج الموازئ (بدون إعادة كتابة نوع G4ThreeVector)
    currentZ = xyzCollimator.z() + hzCollimator + detectorHalfLength;
    xyzDetector = G4ThreeVector(0., 0., currentZ);
