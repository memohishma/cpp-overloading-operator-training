"""
Neutron Energy Spectrum Comparison: Air vs Phantom Interface
==============================================================
Extracts two Geant4 multi-threaded output archives (one "Air"/free-space
case, one "Phantom" case that also contains other scoring ntuples such as
BoneDose/BonePhantom), merges each set's 'output_nt_Detector_t*.csv'
thread files separately, and overlays both neutron energy spectra on a
single publication-quality, IAEA-shaded BNCT plot.
...
def main():
    # -------------------------------------------------------
    # Locate/upload the two zip archives.
    # Edit these two paths if your files are already sitting in the
    # working directory (e.g. after mounting Google Drive) instead of
    # being uploaded interactively.
    # -------------------------------------------------------
    AIR_ZIP = "BSA_2_billion_events.zip"       # <-- edit if your filename differs
    PHANTOM_ZIP = "Phantom_Outputs.zip"        # <-- edit if your filename differs
    ...
    if IN_COLAB:
        ...
        if not os.path.exists(PHANTOM_ZIP):
            uploaded_name = upload_zip_colab("PHANTOM")
            if uploaded_name:
                PHANTOM_ZIP = uploaded_name
    ...
    if not os.path.exists(PHANTOM_ZIP):
        raise FileNotFoundError(
            f"Phantom zip not found at '{PHANTOM_ZIP}'. Edit PHANTOM_ZIP in "
            "the script, upload it, or mount Google Drive and point to its path."
        )

    # -------------------------------------------------------
    # Extract + merge both datasets
    # -------------------------------------------------------
    print("Loading AIR dataset...")
    energies_air_MeV = load_dataset(AIR_ZIP, "extracted_air", label="Air")

    print("Loading PHANTOM dataset...")
    energies_phantom_MeV = load_dataset(PHANTOM_ZIP, "extracted_phantom", label="Phantom")
    ...
    ax.axvspan(1e-9, 0.5e-6, color="red", alpha=0.12, label="Thermal (<0.5 eV)")
    ax.axvspan(0.5e-6, 10e-3, color="green", alpha=0.12, label="Epithermal (0.5 eV\u201310 keV)")
    ax.axvspan(10e-3, 1e1, color="orange", alpha=0.12, label="Fast (>10 keV)")

    # -------------------------------------------------------
    # Two overlaid spectra (step/column histogram style)
    # -------------------------------------------------------
    ax.hist(energies_air_MeV, bins=energy_bins, histtype="step",
            linewidth=2, color="navy",
            label="Detector Interface \u2014 Air")
    ax.hist(energies_phantom_MeV, bins=energy_bins, histtype="step",
            linewidth=2, color="crimson", linestyle="--",
            label="Detector Interface \u2014 Phantom")
    ...
    ax.set_title("Neutron Energy Spectrum at Detector Interface: Air vs. Phantom")
    ...
    output_name = "Neutron_Spectrum_Air_vs_Phantom.png"
